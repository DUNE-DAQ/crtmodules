...
{ 
  ...
  //string cmd = dir_path + "\\/bottomCRTreadout\\/script\\/start_bottomCRTreadout.sh \"bottomCRTreadout\" \"1\" \"1\" \"none\"";
    ...
    string mode = "fcl";
    dunedaq::crtmodules::loadconfig(mode,0,0,filename);                       //read fcl file and load PMT data
    ...
    dunedaq::crtmodules::initializeboard("auto",1000,PMTINI,PMTFIN,online_path);   //Takes baseline data and prepares USBs for writing

    int res = dunedaq::crtmodules::eventbuilder("auto",PMTINI,PMTFIN,online_path);

    dunedaq::crtmodules::starttakedata(PMTINI,PMTFIN);              //Starts taking data
