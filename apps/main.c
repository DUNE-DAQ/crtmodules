...
static char target_dir[300]= "/data0/runs1/DATA/"; //default data folder
...
static void check_data_in() {
  ...
  if (len >= 0) {
    ...
    sprintf(target_dir,"/data0/runs%d%s",disk_number,target_path);
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt (argc, argv, "d:r:g:")) != -1) {
		...
		case 'd': sprintf(target_dir,"/data0/runs%d/%s",disk_number,optarg); strcpy(target_path,optarg);	break;
